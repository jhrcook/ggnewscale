...
bump_aes_scale <- function(scale, new_aes) {
    # gets labels of current aes of `scale` of type `new_aes`
    old_aes <- scale$aesthetics[remove_new(scale$aesthetics) %in% new_aes]
    if (length(old_aes) != 0) {
        # add another "_new" aes
        new_aes <- paste0(old_aes, "_new")
        # replace `old_aes` with `new_aes` with additional "_new" suffix
        ...
        if (is.character(scale$guide)) {
            ...
            scale$guide <- match.fun(paste0("guide_", scale$guide))()
        }

        # adds "_new" suffix to `available_aes` attribute
...
# adds another "_new" to all aes of type `new_aes`
bump_aes_labels <- function(labels, new_aes) {
    # labels of aes of type `new_aes` after removing all of the "_new"s
    old_aes <-  names(labels)[remove_new(names(labels)) %in% new_aes]
    # add another "_new" suffix to these labels
    new_aes <- paste0(old_aes, "_new")
...
# of values in `list`, changes `old` --> `new`
...
# removes the "_new" from "<txt>_new"
remove_new <- function(aes) {
    stringr::str_replace_all(aes, "_new", "")
